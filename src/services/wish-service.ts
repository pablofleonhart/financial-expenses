...
export const loadWishes = () => {
  const localItems = localStorage.getItem('wishItems');
  if (localItems) {
    Object.assign(allWishItems, JSON.parse(localItems));
    filterWishes();
  } else {
  ...
  sortList(allWishItems, wishSettings.column, wishSettings.ascending);
};
